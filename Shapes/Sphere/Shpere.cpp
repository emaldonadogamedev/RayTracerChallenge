...
bool RayTracer::Shapes::Shpere::Intersects(const Ray& ray, IntersectionData& intersectionData) const
{
	...
	const Vector3 sphereToRay = transformedRay.m_origin;// - Vector3(0,0,0);

	//const Vector3 sphereToRay = ray.m_origin - m_position;//

	const float a = transformedRay.m_direction.LengthSquared();
	const float b = 2.0f * transformedRay.m_direction.Dot(sphereToRay);
...
{
	//Old way, assuming that the shape is always at the position in world space, no other transformations
	...
	Vector4 worldSpaceNormal = Matrix4x4(m_invTransformMtx.GetTranspose()) * objSpacePos;
